...
export function Stage4ConveyorBelt({ onNext, playSFX }: Stage4Props) {
  const [suitcaseArrived, setSuitcaseArrived] = useState(false);
  const [collected, setCollected] = useState(false);

  useEffect(() => {
    // Single suitcase arrives
    setTimeout(() => {
      playSFX('beep');
      setSuitcaseArrived(true);
    }, 1200);

    // Auto proceed once it's arrived
    setTimeout(() => {
      playSFX('success');
      setCollected(true);
    ...
    <div className="min-h-screen pt-20 pb-8 px-4">
      <div className="container max-w-lg mx-auto">
        ...
        <div className="relative h-72 mb-8 terminal-card overflow-hidden">
          ...
          <div className="absolute top-12 left-4 right-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${suitcaseArrived ? 'bg-led-green' : 'bg-led-amber animate-pulse'}`} />
              <span className="text-xs text-muted-foreground">
                {suitcaseArrived ? 'Recibida' : 'En proceso'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 mb-6">
          <div className={`terminal-card py-4 text-center transition-all ${suitcaseArrived ? 'border-green-500/50' : ''}`}>
            <div className="text-sm font-semibold mb-1">Valija Grande</div>
            <div className={`text-xs ${suitcaseArrived ? 'text-green-600' : 'text-muted-foreground'}`}>
              {suitcaseArrived ? '✓ Recibida' : 'Esperando...'}
            </div>
          </div>
        </div>
